...
tmux send-keys -t "${SESSION}" "python -u -m differentiation_prediction.yichao_future_expression.train_b2f_strong --output-root '${OUTPUT_ROOT}' --image-size 384 --path-mode original_crop --epochs 1000 --batch-size 8 --grad-accum-steps 2 --architecture pix2pix_unet --base-channels 64 --dropout 0.5 --lr 1.5e-4 --min-lr 1e-6 --balanced-sampler --eval-every 5 --panel-every 20 --save-every 20 --keep-periodic 8 --resume 2>&1 | tee -a '${LOG_FILE}'; status=\${PIPESTATUS[0]}; if [ \${status} -eq 0 ]; then echo STRONG_B2F_FINISHED \$(date -Iseconds) | tee -a '${LOG_FILE}'; else echo STRONG_B2F_FAILED \$(date -Iseconds) status=\${status} | tee -a '${LOG_FILE}'; fi" C-m
