...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--signal-threshold", type=float, default=0.08)
    parser.add_argument("--signal-boost", type=float, default=8.0)
    ...
    parser.add_argument("--scalar-weight", type=float, default=0.10)
...
def b2f_loss(
    ...
    *,
    ...
    weights = args.background_weight + mask + args.signal_boost * signal
    image_loss = (charbonnier(pred - target) * weights).sum() / weights.sum().clamp_min(1.0)
    bce_loss = pixel_bce(logits, signal)
    structure_loss = ssim_loss(pred, target, mask)
    edge_loss = (torch.abs(sobel_edges(pred) - sobel_edges(target)) * (mask + signal)).sum() / (mask + signal).sum().clamp_min(1.0)
