...
\centering
\fbox{%
\begin{minipage}[c][2.3in][c]{0.9\linewidth}
\centering
\textbf{#2}
\end{minipage}}
\caption{#3}
...
\title{OrganoidAgent: An Agent-Orchestrated Platform for Reproducible\
Organoid Dataset Analysis and Computational Phenotyping}
...
\section{Planned Figures (Placeholders)}
\placeholderfigure
{architecture}
{Figure Placeholder: System architecture}
{Overview of OrganoidAgent architecture, including dataset intake, API routing, preview cache, and frontend interaction loop.}

\placeholderfigure
{qc_examples}
{Figure Placeholder: Quality control examples}
{Representative pass/fail quality-control cases for focus blur, illumination drift, and debris artifacts across brightfield frames.}

\placeholderfigure
{seg_track}
{Figure Placeholder: Segmentation and tracking overlays}
{Example organoid boundary masks and immune-cell trajectories overlaid on time-lapse frames with confidence annotations.}

\placeholderfigure
{forecast}
{Figure Placeholder: Growth forecasting}
{Prediction-versus-observation plots for late-stage organoid outcomes from early time-point features, including confidence intervals.}
